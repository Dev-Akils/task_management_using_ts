...
const createTask = (name: string, category: Category) => {
  ...
  const onResizeStart = (taskId: string, edge: 'left'|'right') => {
    ...
    const onMove = (e: PointerEvent) => {
      const target = e.target as HTMLElement | null;
      const tile = target?.closest?.('[data-day]') as HTMLElement | null;
      if(tile){ const d = tile.dataset.day; if(d && resizingRef.current) {
  ...
  return (<>
  <div
      className="min-h-screen w-full flex items-center justify-center  relative p-3 "
    ...
    >
      ...
      <div className=' absolute lg:max-w-[930px] lg:h-[700px]  md:max-w-[650px] md:h-[500px] w-full h-screen   mx-auto p-4 shadow rounded-md ' >
        <div className='flex  justify-between items-center bg-white p-5  mb-4 gap-4 md:gap-2 lg:gap-2'>
          ...
          <div className="grid grid-cols-7  md:w-[600px] md:h-[50px] lg:w-[900px] lg:h-[100px] w-full items-center border-white text-sm md:text-lg lg:text-xl
 ...
 {days.map((d, idx)=>{
          ...
          const inSelection = selectionActive && ((isoDay >= (dragStart || '') && isoDay <= (dragEnd || '')) || (isoDay >= (dragEnd || '') && isoDay <= (dragStart || '')));
          return (
            <div key={isoDay} data-day={isoDay} className="relative">
              <div onPointerDown={(ev)=>handlePointerDownOnTile(isoDay, ev)} className={`h-20 border p-1 relative select-none ${isOther? 'bg-gray-50 text-gray-400' : 'bg-white'}`}>
                <div className="text-xs absolute top-1 right-1">{format(d,'d')}</div>
                {inSelection && <div className="absolute inset-0 bg-orange-200/60" />}
                {/* render task bars that start on this day */}
                <div className="absolute left-0 top-6 right-0 pointer-events-none">
                  {tasksWithLayout.filter(t=>t.startIndex === idx).map(t=> (
